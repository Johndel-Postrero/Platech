window.addEventListener("load", () => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function createSpeedControlInStatsBar() {
      ...
      // Insert speed control before the fragmentation gauge
      const fragmentationGaugeContainer = statsBar.querySelector(".fragmentation-gauge-container")
      statsBar.insertBefore(speedControlContainer, fragmentationGaugeContainer)
    }
    ...
    function updateSimulation() {
      ...
      if (allProcessed) {
        ...
        simulationRunning = false
        document.getElementById("pauseSimulation").disabled = true
        logEvent("All jobs have completed or been rejected")
        startSimulationBtn.disabled = false
        generateTableBtn.disabled = false
      }
    }
    ...
    function processJobs() {
      ...
      jobs.forEach((job) => {
        if (job.status === "waiting" && currentTime >= job.loadingTime && !rejectedJobs.has(job.id)) {
    ...
    function tryAllocateJob(job) {
      ...
      if (freeSlotIndex === -1) {
        // If no suitable slot, check if the job is too large for the total available memory
        const totalFreeMemory = memorySlots
          .filter((slot) => slot.type === "free")
          .reduce((acc, slot) => acc + slot.size, 0)
  
        if (job.size > totalFreeMemory) {
          // Job is too large for the available memory, reject it
          rejectedJobs.add(job.id)
          job.status = "rejected"
          logEvent(`${job.id} (${job.size}KB) rejected - requires ${job.size}KB but only ${totalFreeMemory}KB available`)
          return
        }
  
        // If job could fit but memory is fragmented, log that it's waiting
        logEvent(`${job.id} (${job.size}KB) waiting - not enough contiguous memory`)
        return
      }
